...
source: https://github.com/sickn33/antigravity-awesome-skills/tree/main/skills/frontend-slides
---
...
## Phase 2.5: Font Scale Selection

**REQUIRED STEP before HTML generation.** Never skip.

Generate 3 font scale preview files in `.claude-design/fontsize-previews/` showing the same slide at 3 different scales. Present as visual choices — do not ask users to choose by numbers.

### The 3 Standard Patterns

| Pattern | Title (h1) | Heading (h2) | Body (p) | Bullet |
|---------|-----------|-------------|---------|--------|
| **1: 大胆・インパクト重視** | `clamp(5rem, 9vw, 8rem)` | `clamp(2.8rem, 4.5vw, 4rem)` | `clamp(1.4rem, 2.2vw, 1.8rem)` | `clamp(1.2rem, 1.8vw, 1.5rem)` |
| **2: バランス重視** | `clamp(3.5rem, 7vw, 6rem)` | `clamp(2.2rem, 3.5vw, 3rem)` | `clamp(1.1rem, 1.6vw, 1.35rem)` | `clamp(0.95rem, 1.4vw, 1.15rem)` |
| **3: 情報密度重視** | `clamp(2.4rem, 4.5vw, 4rem)` | `clamp(1.6rem, 2.5vw, 2.2rem)` | `clamp(0.85rem, 1.1vw, 1rem)` | `clamp(0.78rem, 1vw, 0.9rem)` |

### Presentation script

```
フォントスケールを3パターン作成しました：

- **Pattern 1: 大胆・インパクト重視** — タイトル最大8rem、メッセージを大きく打ち出す
- **Pattern 2: バランス重視** — タイトル最大6rem、読みやすさとインパクトのバランス
- **Pattern 3: 情報密度重視** — タイトル最大4rem、情報量が多いスライドに適切

.claude-design/fontsize-previews/ で確認してください。
どのパターンにしますか？（または「1と2の中間で」など組み合わせ可）
```

Record the user's selected pattern as the baseline. Apply consistently across all slides.
...
## Phase 4.5: Pre-Delivery Self-Review

**REQUIRED before showing the HTML to the user.** Run this checklist silently and fix all issues before presenting.

### Checklist

**Font size compliance:**
- [ ] All font sizes match the agreed pattern (Phase 2.5)
- [ ] No element uses a font size smaller than the agreed baseline
- [ ] Cards/badges/labels are not so small they're unreadable

**Japanese line break audit (every text element):**
- [ ] No single-character orphan lines (e.g., 「す」alone on a line)
- [ ] No 2-character orphan lines (e.g., 「する」alone) in titles or headings
- [ ] All text elements have `word-break: keep-all; overflow-wrap: break-word;`
- [ ] Explicit `<br>` added where needed to control break points

**Layout alignment:**
- [ ] Elements in the same flex row have equal width (use `flex: 1` or explicit widths)
- [ ] Grid cards have equal height (`display: flex; flex-direction: column; height: 100%`)
- [ ] No excessive whitespace — if space remains, consider increasing font size
- [ ] `display: flex` used for full-width containers (not `inline-flex`)

**Navigation:**
- [ ] Nav dots / pagers are hidden by default (`display: none`) unless explicitly requested

Report the self-review result to the user as a brief summary BEFORE asking them to check the HTML:
```
【自己レビュー完了】
✅ フォントサイズ: Pattern X 基準値に準拠
✅ 孤立改行: なし
✅ レイアウト整合: カード高さ統一、同列要素の幅統一
✅ ナビドット: 非表示
...
- **Prevent mid-word line breaks**: Add `word-break: keep-all; overflow-wrap: break-word;` to ALL text elements
- **No orphan single characters**: After increasing font sizes, check for single-character orphan lines and control with explicit `<br>` tags
- **Consistent card line counts**: When using grid cards, use `<br>` to make all cards the same number of lines
- **Impact lines**: Center-aligned (`text-align: center`), gradient text, large font, `white-space: nowrap` if text is short enough
- **Adjust layout simultaneously with font size**: Never increase font size without also checking and fixing line wrapping
- **Equal width for same-row elements**: Sibling elements in a flex row must have equal width — use `flex: 1` or explicit equal widths. Never let one item be wider than its siblings.
- **Equal height for grid cards**: Apply `display: flex; flex-direction: column; height: 100%` to all cards in a grid so they stretch to equal height.
- **Maximize font size within available space**: If whitespace remains at the right or bottom of a slide, increase font size rather than leaving dead space. Less whitespace + larger text > smaller text + large whitespace.
- **Use `display: flex` for full-width containers**: Do not use `display: inline-flex` for elements that should span the full row width.
- **Navigation dots are hidden by default**: Set `.nav-dots { display: none }` unless the user explicitly requests pager navigation.

### Content Rules

- **No conceptual overlap between slides**: Each slide's impact line must not duplicate the content of adjacent slides
- **Speak in outcomes, not tools**: Use "全社展開" not "Webツール化", "本番運用" not "Claude Code実装"

---

## Post-Completion Retrospective

**REQUIRED after the final presentation is approved.** Run this workflow before closing the session.

### Step 1: Collect all feedback from this session

Review all user corrections and comments during the session. For each issue:
- What was the specific problem? (concrete)
- What is the underlying principle? (abstract, reusable)

### Step 2: Present abstracted rules to user

```
【振り返り】今回のセッションで学んだルール候補です。
スキルへの反映可否を教えてください：
...
- BAD: "S6のカード幅を揃えた" (too specific)
- GOOD: "同じ行に並ぶflexアイテムはflex:1で横幅統一" (generalizable)
- BAD: "フォントが小さい" (symptom, not rule)
- GOOD: "余白がある限りフォントを大きくすることを優先する" (actionable principle)

### Step 3: Apply approved rules

For each approved rule:
- Add to the relevant section of this SKILL.md
- If it overlaps an existing rule, strengthen/update that rule instead
